...
const getPostLike = (filterFields = {}, fields = []) => {
  let queryParams = "";
  ...
  return axios.get(API_URL + "getPostLike" + queryParams);
};

const createPostLike = (postData) => {
  return axios.post(API_URL + "createPostLike", {...postData, created_by: user?.user_id} );
};

const updatePostLike = (id, updatedData) => {
  return axios.put(API_URL + "updatePostLike/" + id, {...updatedData, modified_by: user?.user_id});
};

const deletePostLike = (id) => {
  return axios.delete(API_URL + "deletePostLike/" + id + "/"+ user?.user_id);
};

const searchPostLikes = ({ page = 1, limit = 10, search = "", filters = {} }) => {
  ...
  return axios.get(API_URL + "searchPostLikes" + queryParams);
};

const getLikeCount = async (postId) => {
  try {
    const response = await axios.get(`${API_URL}/getLikeCount/${postId}`); 
